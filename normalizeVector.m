function [normalized_vector_idx, normalized_vector_val] = normalizeVector...
                                                    (vector_idx,vector_val,...
                                                     normalization_grid,...
                                                     varargin)
% Input:
%     [n x 1] vector of indices 
%     [n x 1] vector of values
%     [1 x m] vector of normalization grid 
% output:
%     [1 x m] normalized vector of segment indices 
%     [1 x m] normalized vector of segment values
%   it creates a new vector that has as many elements as specified by 
%   the normalization grid and contains samples from the original segment 
%   values
% 
%%
normalized_vector_idx = nan(1,length(normalization_grid));
normalized_vector_val = nan(1,length(normalization_grid));
%% get index of all values in the segment that are not NAN
ind_non_nan_vals_in_seg = find(~isnan(vector_val));

%% create mormalized time vector for segment
stepsize = normalization_grid(2) - normalization_grid(1);
max_val = length(vector_idx)-1;
range= 0:max_val;
...
non_nan_val_in_seg = vector_val(ind_non_nan_vals_in_seg);
orig_idx_non_nan_val_in_seg = vector_idx(ind_non_nan_vals_in_seg);
...
these_cols = find(ismember(normalization_grid,unique_t_grid_with_non_nan_val));
...
if any(diff(t_grid_with_non_nan_val)==0)
    ...
    for m=1:length(time_cells_with_many_vals)
        ...
        notEmptyButInvalidIndex =~isempty(varargin) && varargin{1} <= 0;
        if isempty(varargin)|| notEmptyButInvalidIndex
        ...
        col1 = find(ismember(normalization_grid,time_cells_with_many_vals(m)));
